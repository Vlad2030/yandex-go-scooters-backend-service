...
fn generate_objects(
    ...
    start_pos_y: Float64,
    count: Int,
) -> ListLiteral[ListLiteral[Float64, Float64]]:
    let objects: ListLiteral = []
    ...
    for obj in range(count):
        ...
        objects.get[obj, ListLiteral[Float64, Float64]]: ListLiteral[Float64, Float64] = [pos_x, pos_y]
...
) -> ListLiteral[ListLiteral[Float64]]:
...
let pos_points: ListLiteral[ListLiteral[Float64]] = calculate_shape_points(pos1, pos2)
let shape_end: Float64 = perf_counter() - shape_start

let search_start: Float64 = perf_counter()
let founded_objects: ListLiteral[ListLiteral[Float64]] = search_objects(objects, pos_points)
let search_end: Float64 = perf_counter() - search_start

fn main():
    print("GENERATION TIME")
    print("objects time:\t", objects_end, "secs")
    print("shape time:\t", shape_end, "secs")
    print("search time:\t", search_end, "secs")
    print()
    print("SEARCH POS")
    print("pos1:\t", pos1)
    print("pos2:\t", pos2)
    print()
    print("SHAPE")
    # [print("pos{number}:\t{point}") for number, point in enumerate(pos_points, 1)]
    # print()
    # print("OBJECTS: {len(objects)}")
    # if len(objects) <= 20:
    #     [print("{number}.\tobj:\t{obj}") for number, obj in enumerate(objects, 1)]
    # print()
    # print("FOUNDED OBJECTS ({len(founded_objects)})")
    # if len(founded_objects) <= 20:
    #     [print("{number}.\tobj:\t{obj}") for number, obj in enumerate(founded_objects, 1)]

    let python_end: Float64 = perf_counter() - mojo_start

    print()
    print("EXECUTION TIME")
    print("mojo time:\t", python_end, "secs")